...
- **Java**: Downloads a specific OpenJDK version + all Maven (3.9.16) and all Gradle versions; patches `runtimes-config.json` to fix JDK path mismatches, and `tools-config.json` (`patch_tools_config_maven`) to fix the Maven version/path mismatch — veecli's bundled config hardcodes its lone `MvnProvider` entry to `maven-3.8.9`, which doesn't match the `MAVEN_VERSION` we install; without this patch veecli silently falls back to whatever `mvn` is on the runner's PATH. Java 15+ supports aarch64; Java 25 is x64-only.
