...
0. `write_fix_config` writes the fix config to **four paths** — `fix.yml` and `fix.yaml`, in both `<veecli_dir>` and `~/veecli/` — plus three files the verify hook needs: `lineaje-verify.sh`, `lineaje-verify.npmrc`, and `lineaje-verify-pip.conf`. Only the fix.yaml keys that apply when applying **without a PR** are emitted — `pull_request`, `reviewers` and `credentials` are omitted, the last deliberately, since it would put SCM tokens on disk for no benefit.
  ...
  The `connect_to_fortknox` action input (default `true`) lets a customer opt out of Fortknox entirely. Its primary mechanism, in `_run_fix_plan`: right after the fix plan is fetched, every `plan_details` entry whose `suggested_purl` contains `-lineaje-N` is dropped *before* `apply_fix_left_plan` ever submits the plan, so a Fortknox-only component never gets a patch task queued for it in the first place. This was chosen over the original design — submit everything and let the verify hook reject the unresolvable ones at npm-install time — after observing in practice that submitting components guaranteed to fail install could jeopardize *unrelated* curated fixes queued in the same `veecli fix --poll-tasks` batch (a curated component that should have installed cleanly came back unpatched when Fortknox-only siblings were also submitted and failed, for reasons internal to that batching/orchestration that aren't visible or controllable from this action's side). Pre-filtering avoids ever creating the doomed tasks. `lineaje-verify.sh` also still supports `--connect-to-fortknox false` (falling back to `registry.npmjs.org`, skipping the `GOS_PREMIUM_NPM_REGISTRY` requirement) as a second, defense-in-depth layer, and `write_fix_config` still skips writing the npmrc in that case — but the verify-hook layer should now rarely if ever actually see a Fortknox-only purl, since the plan_details filter runs first. This is Node-only: Python's side (`pip_extra_index_url`/`pip.conf`) never had a Fortknox/GOS index to disconnect from, so the flag has no effect there.
...
1. `apply_fix_left_plan` POSTs to `/api/v1/explain` with `query="Apply fix left plan without pr"` and `metadata.components` set to the fix plan's `plan_details` **verbatim** (after the `connect_to_fortknox=false` pre-filter above, if it ran). The backend checks each `suggested_purl` against the GOS artifactory (Lineaje-rebuilt versions like `pkg:npm/helmet@2.3.0-lineaje-01`) and queues patch tasks. Beyond that pre-filter, this action never queries the artifactory directly and does no other client-side filtering of `plan_details` — deliberately, so an availability filter can't drift from the backend's. The `connect_to_fortknox` filter is different in kind: it's a purl-suffix check on data the backend already returned, not a guess about what's actually stocked in Fortknox.
